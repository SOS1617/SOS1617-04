   ...
   dbAdrian.find({}).skip(off).limit(limite).toArray(function(err, sArea) {
    if (err) {
     ...
     var filted = sArea.filter((stat) => {
      if ((province == undefined || stat.province == province) && (year == undefined || stat.year == year) && (year == undefined || stat.year == year) && (areaS == undefined || stat.areaS == areaS) && (productionS == undefined || stat.productionS == productionS)) {
       return stat;
      }
     });
     if (filted.length > 0) {
